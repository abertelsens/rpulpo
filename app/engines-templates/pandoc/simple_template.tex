...
\documentclass$if(fontsize)$[$fontsize$,$endif$$if(papersize)$$papersize$paper$endif$]{scrartcl}
...
% Font:
% If the mainfont variable is set we use it, otherwise the default will be Times Roman.
%$if(mainfont)$
...
%\setromanfont[Mapping=tex-text,Numbers=OldStyle]{$mainfont$} 
...
% define un nuevo comando para texto pequenio. Usar \smallfont{este texto sera pequenio}
\newcommand{\smallfont}[1]{{%
  \fontsize{9pt}{12pt}\normalfont #1%
}}

% Latex does not indent the first paragraph after a section. This line corrects that behaviour
...
% Prevent sections being numbered
